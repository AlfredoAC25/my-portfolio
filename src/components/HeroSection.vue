...
const script = [
  ...
  { type: 'out', text: 'Alfredo Arámburo — Backend Developer' },
  { type: 'cmd', text: 'cat stack.txt' },
  { type: 'out', text: 'Laravel · Golang · Java · Vue.js' },
  { type: 'cmd', text: './experiencia.sh --resumen' },
  { type: 'out', text: '3 años construyendo ERPs, CRMs, facturación electrónica y plataformas de movilidad.' },
]
...
onMounted(async () => {
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    renderedLines.value = script.map((l) => ({ ...l }))
    done.value = true
    return
  }
    ...
    <div class="container hero-grid">
      ...
      <div class="terminal reveal">
        ...
        <div class="terminal-body">
          <div v-for="(line, i) in renderedLines" :key="i" :class="['line', line.type]">
            <span v-if="line.type === 'cmd'" class="prompt">alfredo@backend:~$</span>
            <span>{{ line.text }}</span>
          </div>
...
.terminal {
  ...
  box-shadow: var(--shadow-card); overflow: hidden; font-family: var(--font-mono); font-size: 0.85rem;
}
.terminal-bar { display: flex; align-items: center; gap: 8px; padding: 12px 16px; border-bottom: 1px solid var(--border); background: var(--bg-elevated); }
.dot { width: 10px; height: 10px; border-radius: 50%; }
.dot.red { background: #e5635b; } .dot.yellow { background: #e3b341; } .dot.green { background: #57ab5a; }
.terminal-title { margin-left: 8px; color: var(--text-faint); font-size: 0.75rem; }
.terminal-body { padding: 20px; min-height: 220px; }
.line { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 10px; line-height: 1.5; }
.line.out { color: var(--text-dim); }
.prompt { color: var(--accent-cool); white-space: nowrap; }
.cursor { display: inline-block; width: 8px; height: 16px; background: var(--accent); animation: blink 1s step-end infinite; }
